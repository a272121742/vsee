...
export default {
  ...
  computed: {
          ...
          formatter (value) {
            ...
            if (self.legendArr.length > 0) {
              times(length, (i) => {
                ...
                ${self.$t('info.actualValue')}:${Math.round(value[0].value[1 + 3 * self.legendArr[i]] * 100) / 100}&nbsp;
                ${self.$t('info.targetValue')}:${Math.round(value[0].value[2 + 3 * self.legendArr[i]] * 100) / 100}&nbsp;
                <div>
                `;
              });
            } else {
              times(length, (i) => {
                html += `
                <div>${value[i].marker}${value[i].seriesName}:
                ${self.$t('info.actualValue')}:${Math.round(value[0].value[1 + 3 * i] * 100) / 100}&nbsp;
                ${self.$t('info.targetValue')}:${Math.round(value[0].value[2 + 3 * i] * 100) / 100}&nbsp;
                <div>
                `;
              });
            }
    ...
    storeFN () {
      return this.$store.state[`process-analysis`].pdiFdrParams;
    },
  },
  watch: {
    // 监听筛选条件数据的变化
  ...
  methods: {
    ...
    async fetch () {
      ...
      this.newData = [];
      const echartsData = await this.getEcharts(this.params).then((data) => data).catch(() => {
          ...
          // 主数据序列
            return {
              ...
              label: {
                ...
                distance: 20,
                formatter (value) {
                  return value.value[1 + 3 * (i - 1)] === 0 ? '' : `${Math.round(value.value[1 + 3 * (i - 1)] * 100) / 100}`;
                },
              },
              barWidth: 20,
            };
