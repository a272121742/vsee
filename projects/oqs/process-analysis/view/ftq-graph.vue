...
export default {
  ...
  computed: {
    ...
    storeFN () {
      return this.$store.state[`process-analysis`].ftqParams;
    },
  },
  watch: {
    // 监听筛选条件数据的变化
  ...
  methods: {
    ...
    async fetch (config = {}) {
      ...
      const echartsData = await this.getEcharts({ ...this.params, ...config })
        .then((data) => data)
        .catch(() => {
          this.$message.error(`error-${this.$t('info.echarts-data-error')}`, 0.4);
          return false;
        });
      this.spinning = false;
      if (echartsData === null) {
      ...
      if (echartsData.length) {
          ...
          // 主数据序列
            return {
              ...
              label: {
                ...
                distance: 20,
                formatter (value) {
                  return `${Math.round(value.value[1 + 3 * (i - 1)] * 100)}%`;
                },
              },
              barWidth: 20,
            };
        ...
        this.$message.warning(this.$t('NoData'), 0.4);
      }
    },
    // 图例点击事件
    legendClick (e) {
