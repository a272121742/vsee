...
import { drlColumns } from '~~/model/analyze.js';
...
export default {
  components: {
    DrlGraph: () => import('../view/drl-graph.vue'),
  ...
  activated () {
    ...
    if (window.location.href.indexOf('?') > 0) {
      ...
      this.query();
      this.$router.push('/process-analysis/drl');
  ...
  methods: {
    ...
    query () {
      ...
      const drlParams = { ...this.echartsParams };
      this.$store.commit('process-analysis/set', { drlParams });
    },
    // 参数
    requestParams () {
      ...
      // 图表的code
      params.code = `oqs_drl_l1_${this.dateCode}`;
      // 工厂权限
    ...
    async requestTable (request) {
      ...
      params.code = `oqs_drl_l1_${this.dateCode}_list`;
      this.loading = true;
