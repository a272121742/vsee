          ...
          <template slot-scope="text,row">
            ...
            <a-tooltip v-if="['nmrtr_value'].includes(col.dataIndex)">
              ...
              <a
                ...
                href="javascript:;"
                @click="handlePdi(row,isDefect=true)"
              >
                {{ text }}
              </a>
            </a-tooltip>
            ...
            <a-tooltip v-else-if="['dnmtr_value'].includes(col.dataIndex)">
              ...
              <a
                style="text-decoration:underline"
                href="javascript:;"
                @click="handlePdi(row,isDefect=false)"
              >
                {{ text }}
              </a>
            </a-tooltip>
...
import { pdiDdrColumns } from '~~/model/analyze.js';
...
export default {
  components: {
    ...
    PdiDdrGraph: () => import('../view/pdi-ddr-graph.vue'),
  },
  mixins: [
  ...
  activated () {
    if (window.location.href.indexOf('?') > 0) {
      ...
      this.query();
      this.$router.push('/process-analysis/pdi-ddr');
    }
  },
  methods: {
    eSpingFn (data) {
    ...
    handlePdi (row) {
      ...
      this.$router.push({
        path: '/process-trace/pdi-trace',
        query: params,
      });
    },
    query () {
      // 表格渲染
      const params = this.requestParams('table');
      ...
      const pdiDdrParams = { ...this.echartsParams };
      this.$store.commit('process-analysis/set', { pdiDdrParams });
    },
    // 参数
    requestParams () {
      ...
      // 图表的code
      params.code = `oqs_pdi_ddr_l1_${this.dateCode}`;
      // 工厂权限
    ...
    async requestTable (request) {
      ...
      params.code = `oqs_pdi_ddr_l1_${this.dateCode}_list`;
      this.loading = true;
      ...
      this.tableData = tableData.list.filter(((item) => {
        if (item.check_stock_car_num_rate !== 0) {
          if (item.check_stock_car_num_rate === 1) {
            item.check_stock_car_num_rate = '100%';
          } else {
            item.check_stock_car_num_rate = `${Math.round(item.check_stock_car_num_rate * 100)}%`;
          }
        }
        if (item.target !== 0) {
          if (item.target === 1) {
