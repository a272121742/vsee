...
import { pdiFdrColumns } from '~~/model/analyze.js';
...
export default {
  components: {
    ...
    PdiFdrGraph: () => import('../view/pdi-fdr-graph.vue'),
  },
  mixins: [
  ...
  activated () {
    if (window.location.href.indexOf('?') > 0) {
      ...
      this.query();
      this.$router.push('/process-analysis/pdi-fdr');
    }
  },
  methods: {
    // 图表的loading
    ...
    query () {
      ...
      const pdiFdrParams = { ...this.echartsParams };
      this.$store.commit('process-analysis/set', { pdiFdrParams });
    },
    // 参数
    requestParams () {
      ...
      // 图表的code
      params.code = `oqs_pdi_fdr_l1_${this.dateCode}`;
      // 工厂权限
    ...
    async requestTable (request) {
      ...
      params.code = `oqs_pdi_fdr_l1_${this.dateCode}_list`;
      this.loading = true;
