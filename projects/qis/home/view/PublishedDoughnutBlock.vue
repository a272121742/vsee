  ...
  >
    ...
    <div class="chart-panel">
      <div class="chart-status">
        {{ $t('published_block.unclose') }}
      </div>
      <div class="chart-size">
        <span>{{ unCloseNumber }}</span> {{ $t('published_block.count') }}
      </div>
    </div>
    <ratio-graph
      :data="data"
      :title="$t('published_block.fail_ratio')"
      :ratio="ratio"
    >
    </ratio-graph>
  </a-card>
...
export default {
  ...
  methods: {
    ...mapActions(['creatorCount']),
    init () {
      this.fetch();
    },
    fetch () {
      const user = this.$store.getters.getUser();
      this.creatorCount({ userId: user.id }).then(this.load);
    },
    load (data = {}) {
