<template>
  <div id="components-form-demo-advanced-search">
    <div style="z-index:6001;">
      <!-- 问题分裂  创建/编辑-->
      <problem-fission
        ref="fissionApplication"
      >
      </problem-fission>
      <!-- 问题分裂  创建/编辑-->
      <problem-fission-view
        ref="fissionView"
      >
      </problem-fission-view>
      <!-- 问题裂变回退 -->
    ...
    >
      ...
      >
        ...
        <a-row v-if="statusCode.maxDiamonds<4">
          <a-col :span="22">
            <!-- 标准要求 -->
            <a-form-item
    ...
    >
      <div class="top-buttons">
        ...
        <div class="rightButton">
          <a-button
            v-if="pagePermission.button_allocation_3&&!isPission"
            html-type="submit"
            @click="showModal"
          >
            <!-- 再分配 -->
          ...
          <a-button
            v-if="pagePermission.button_submit_3&&isStagedFlag!=='1'&&(!isPission||(isPission&&(stepCurrent>4||statusCode.statusNewCode<300104)))"
            ref="commitButton"
          ...
          <prevent-button
            v-if="pagePermission.button_commit_3&&isStagedFlag!=='1'&&(!isPission||(isPission&&stepCurrent>4||statusCode.statusNewCode<300104))"
            ref="saveButton"
            bind="both"
          ...
          <a-dropdown-button
            v-if="(pagePermission.button_staged_3&&isStagedFlag!=='1')||(pagePermission.button_staged_3&&isStagedFlag!=='0')||(pagePermission.button_specialClose_3&&(!isPission||(isPission&&stepCurrent>4)))||pagePermission.button_statusBack_3||(pagePermission.button_repeat_3&&isStagedFlag!=='1'&&(!isPission||(isPission&&stepCurrent>4)))||(pagePermission.button_fissionApply_3&&(statusCode.statusNewCode==='300103'||!isPission))||pagePermission.button_cancelFission_3"
            style="marginLeft: 8px"
          >
            ...
            <a-menu slot="overlay">
              ...
              <!-- 恢复 -->
              <a-menu-item
                v-if="pagePermission.button_staged_3&&isStagedFlag!=='0'"
                key="2"
                @click="temporary('0')"
              >
              ...
              <!-- 重复关联 -->
              <a-menu-item
                v-if="pagePermission.button_repeat_3&&isStagedFlag!=='1'&&(!isPission||(isPission&&stepCurrent>4))"
                key="3"
                @click="duplicateFunction()"
              >
                {{ $t('issue_action.duplicateAssociation') }}
              </a-menu-item>
              <!-- 更换SM -->
              <a-menu-item
                v-if="pagePermission.button_transferSm_3"
                key="4"
                @click="replaceSMFunction()"
              >
                {{ $t('issue_action.replaceSM') }}
              </a-menu-item>
              <!-- 问题回退 -->
              <a-menu-item
                v-if="pagePermission.button_statusBack_3"
                key="5"
                @click="rollback()"
              >
              ...
              <!-- 特殊关闭 -->
              <a-menu-item
                v-if="pagePermission.button_specialClose_3&&(!isPission||(isPission&&stepCurrent>4))"
                key="6"
                @click="specialClose()"
              >
              ...
              <!-- 问题裂变 -->
              <a-menu-item
                v-if="pagePermission.button_fissionApply_3&&(statusCode.statusNewCode==='300103'||!isPission)"
                key="7"
                @click="fissionCreate"
              >
              ...
              <!-- 取消裂变 -->
              <a-menu-item
                v-if="pagePermission.button_cancelFission_3"
                key="8"
                @click="cancelFission"
              >
                {{ $t('issue_action.problemFissionCancel') }}
              </a-menu-item>
            </a-menu>
          </a-dropdown-button>
          <!-- 删除按钮 -->
    ...
    <div class="formConetnt">
                  ...
                  <a-col :span="6">
                    ...
                    <a-form-item :label="$t('issue.hardwareVersion')">
                      <p
                        ...
                        :title="detailList.hardwareVesrsion"
                      >
      ...
      >
        ...
        >
          ...
          <div class="ant-advanced-search-form">
              ...
              <a-steps :current="stepCurrent">
                ...
                >
                  ...
                  <span class="new-steps-icon">
                    <img
                      v-if="((index < stepCurrent||statusCode.issueClosed||(record.isSign=='0' && statusCode.statusNewCode==700200))&&!statusSpecialClose)||((index < specialCloseNode||index==6)&&statusSpecialClose)"
            ...
            <div
              v-if="((pagePermission.A0_3_2&&pagePermission.A0_1_1)||pagePermission.A1_2_2||(pagePermission.A2_4_2&&!(pagePermission.A2_1_3))||(pagePermission.A3_4_2&&!pagePermission.A3_1_3)||(pagePermission.A4_4_2&&!pagePermission.A4_1_3)||pagePermission.A5_6_2||pagePermission.A6_3_2||(pagePermission.A2_6_2&&isPission)||(pagePermission.A3_5_2&&isPission)||(pagePermission.A4_5_2&&isPission))&&(backCurrent === stepCurrent)"
              class="Dcontent D0back"
            >
              <a-row>
                <a-col
                ...
                >
                  ...
                  <!-- 问题原因正在分析填写中 -->
                  <p v-if="pagePermission.A2_4_2&&!isPission">
                    {{ $t('issue_reason.analysising') }}
                  </p>
                  <!-- 子问题原因分析中 -->
                  <p v-if="pagePermission.A2_6_2&&isPission">
                    {{ $t('issue_reason.sub_analysising') }}
                  </p>
                  <!-- 问题措施正在制定中 -->
                  <p v-if="pagePermission.A3_4_2&&!isPission">
                    {{ $t('issue_reason.sovling') }}
                  </p>
                  <!-- 子问题措施制定中 -->
                  <p v-if="pagePermission.A3_5_2&&isPission">
                    {{ $t('issue_reason.sub_sovling') }}
                  </p>
                  <!-- 该问题措施实施填写中 -->
                  <p v-if="pagePermission.A4_4_2&&!isPission">
                    {{ $t('issue_reason.implementing') }}
                  </p>
                  <!-- 子问题措施实施中 -->
                  <p v-if="pagePermission.A4_5_2&&isPission">
                    {{ $t('issue_reason.sub_implementing') }}
                  </p>
                  <!-- 该问题效果验证内容填写中 -->
                      ...
                      <div class="processUl">
                        <ul>
                          <li class="clearfix">
                            ...
                            <!-- 责任部门 -->
                            <span style="margin-right:66px;">{{ $t('issue_workflow.D1.resposibleDepartment') }}</span>
                          ...
                          <li class="clearfix">
                            ...
                            <a-form-item style="display:inline-block">
                              <net-single-tree-select
                                v-decorator="['responsibleDepartmentId1',{rules:[{required:true, message:$t('search.please_select') }]}]"
                                :transform="responseDeptTreeTransform"
                                :placeholder="$t('search.please_select')"
                                :query="{ nameCode: '${value}' }"
                                :delay="false"
                                allow-clear
                                style="width:272px;height:32px;line-height:32px;"
                                show-search
                                url="/masterdata/v1/workflowgroupname/tree?typeCode=RESPONSIBLE_DEPARTMENT"
                                @change="DeptChange(1)"
                              />
                            ...
                            <a-form-item style="display:inline-block">
                              <net-select
                                ...
                                :placeholder="$t('search.please_select')"
                                :disabled="!record.responsibleDepartmentId1"
                                url="/masterdata/v1/workflowgroupmember"
                                :query="{workflowGroupNameCode:record.responsibleDepartmentId1}"
                                :cache="false"
                                :delay="false"
                                :transform="userOption"
                                allow-clear
                                style="width:272px;height:32px;"
                                @change="(value) => worlkChampionOrProposer(value, 'diamondOwner1',false)"
                              >
                              </net-select>
                            </a-form-item>
                          </li>
                          ...
                          <li class="clearfix">
                            ...
                            <a-form-item style="display:inline-block">
                              <net-single-tree-select
                                v-decorator="['responsibleDepartmentId4', {rules:[{required:true, message:$t('search.please_select') }]}]"
                                :transform="responseDeptTreeTransform"
                                :placeholder="$t('search.please_select')"
                                :query="{ nameCode: '${value}' }"
                                :delay="false"
                            ...
                            <a-form-item style="display:inline-block">
                              <net-select
                                ...
                                :placeholder="$t('search.please_select')"
                                :disabled="!record.responsibleDepartmentId4"
                                url="/masterdata/v1/workflowgroupmember"
                                :query="{workflowGroupNameCode:record.responsibleDepartmentId4}"
                                :cache="false"
                                :delay="false"
                                :transform="userOption"
                                allow-clear
                                style="width:272px;height:32px;"
                                @change="(value) => worlkChampionOrProposer(value, 'diamondOwner4',false)"
                              >
                              </net-select>
                            </a-form-item>
                          </li>
                          <li class="clearfix">
                            ...
                            <a-form-item style="display:inline-block">
                              <net-single-tree-select
                                v-decorator="['responsibleDepartmentId5', {rules:[{required:true, message:$t('search.please_select') }]}]"
                                :transform="responseDeptTreeTransform"
                                :placeholder="$t('search.please_select')"
                                :query="{ nameCode: '${value}' }"
                                :delay="false"
                            ...
                            <a-form-item style="display:inline-block">
                              <net-select
                                ...
                                :placeholder="$t('search.please_select')"
                                :disabled="!record.responsibleDepartmentId5"
                                url="/masterdata/v1/workflowgroupmember"
                                :query="{workflowGroupNameCode:record.responsibleDepartmentId5}"
                                :cache="false"
                                :delay="false"
                                :transform="userOption"
                                allow-clear
                                style="width:272px;height:32px;"
                                @change="(value) => worlkChampionOrProposer(value, 'diamondOwner5',false)"
                              >
                              </net-select>
                            </a-form-item>
                          </li>
                          <li class="clearfix">
                            ...
                            <a-form-item style="display:inline-block">
                              <net-single-tree-select
                                v-decorator="['responsibleDepartmentId6', {rules:[{required:true, message:$t('search.please_select') }]}]"
                                :transform="responseDeptTreeTransform"
                                :placeholder="$t('search.please_select')"
                                :query="{ nameCode: '${value}' }"
                                :delay="false"
                            ...
                            <a-form-item style="display:inline-block">
                              <net-select
                                ...
                                :placeholder="$t('search.please_select')"
                                :disabled="!record.responsibleDepartmentId6"
                                url="/masterdata/v1/workflowgroupmember"
                                :query="{workflowGroupNameCode:record.responsibleDepartmentId6}"
                                :cache="false"
                                :delay="false"
                                :transform="userOption"
                                allow-clear
                                style="width:272px;height:32px;"
                                @change="(value) => worlkChampionOrProposer(value, 'diamondOwner6',false)"
                              >
                              </net-select>
                            </a-form-item>
                          </li>
                          <li class="clearfix">
                            ...
                            <a-form-item style="display:inline-block">
                              <net-single-tree-select
                                v-decorator="['responsibleDepartmentId7', {rules:[{required:true, message:$t('search.please_select') }]}]"
                                :transform="responseDeptTreeTransform"
                                :placeholder="$t('search.please_select')"
                                :query="{ nameCode: '${value}' }"
                                :delay="false"
                            ...
                            <a-form-item style="display:inline-block">
                              <net-select
                                ...
                                :placeholder="$t('search.please_select')"
                                :disabled="!record.responsibleDepartmentId7"
                                url="/masterdata/v1/workflowgroupmember"
                                :query="{workflowGroupNameCode:record.responsibleDepartmentId7}"
                                :cache="false"
                                :delay="false"
                                :transform="userOption"
                                allow-clear
                                style="width:272px;height:32px;"
                                @change="(value) => worlkChampionOrProposer(value, 'diamondOwner7',false)"
                              >
                              </net-select>
                            </a-form-item>
                          </li>
                    ...
                    <div class="processUl">
                      <ul>
                        <li class="clearfix">
                          ...
                          <span>{{ $t('issue_workflow.D1.7thDiamond_flow') }} </span>
                          <!-- 责任部门 -->
                          <span>{{ $t('issue_workflow.D1.resposibleDepartment') }}</span>
                          <span>{{ $t('issue_workflow.D1.analysis_champion') }}</span>
                        </li>
                        <li class="clearfix">
                          <!-- 第1钻： 过程是否正确？ -->
                          <span>{{ $t('issue_workflow.D1.1stDiamond') + $t('colon') + $t('issue_workflow.D1.rightProcess') + $t('question_mark') }}</span>
                          <span>{{ issueDefinitionData.responsibleDepartmentName1 }}</span>
                        ...
                        <li class="clearfix">
                          <!-- 第4钻 物料规格检查? -->
                          <span>{{ $t('issue_workflow.D1.4thDiamond') + $t('colon') + $t('issue_workflow.D1.specification') + $t('question_mark') }}</span>
                          <span>{{ issueDefinitionData.responsibleDepartmentName4 }}</span>
                          <span>{{ issueDefinitionData.diamondOwner4 }}</span>
                        </li>
                        <li class="clearfix">
                          <!-- 第5钻 过程变更 -->
                          <span>{{ $t('issue_workflow.D1.5thDiamond') + $t('colon') + $t('issue_workflow.D1.processChange') + $t('question_mark') }}</span>
                          <span>{{ issueDefinitionData.responsibleDepartmentName5 }}</span>
                          <span>{{ issueDefinitionData.diamondOwner5 }}</span>
                        </li>
                        <li class="clearfix">
                          <!-- 第6钻 部件变更 -->
                          <span>{{ $t('issue_workflow.D1.6thDiamond') + $t('colon') + $t('issue_workflow.D1.partChange') + $t('question_mark') }}</span>
                          <span>{{ issueDefinitionData.responsibleDepartmentName6 }}</span>
                          <span>{{ issueDefinitionData.diamondOwner6 }}</span>
                        </li>
                        <li class="clearfix">
                          <!-- 第7钻 是否极端复杂问题 -->
                          <span>{{ $t('issue_workflow.D1.7thDiamond') + $t('colon') + $t('issue_workflow.D1.complex') + $t('question_mark') }}</span>
                          <span>{{ issueDefinitionData.responsibleDepartmentName7 }}</span>
            ...
            <div
              v-if="(stepCurrent===2&&backFlag===false)&&(((pagePermission.A2_1_3&&!isPission)||(pagePermission.A2_5_2&&isPission&&statusCode.statusNewCode<300104&&statusCode.statusNewCode>300100))||(pagePermission.A2_3_2&&(backType==='003'||backType==='004'||backType==='005')))"
            ...
            >
              ...
              <div
                v-if="(pagePermission.A2_5_2||pagePermission.A2_3_2)&&(backType==='003'||backType==='004'||backType==='005')"
                class="examineResult"
              >
                <!-- 审核结果 -->
                <div v-if="backType==='003'">
              ...
              <div
                v-if="(stepCurrent===backCurrent)&&(pagePermission.A2_5_2&&isPission)&&statusCode.statusNewCode<300104&&statusCode.statusNewCode>300100"
                class="fissionContent clearfix"
              >
                <p>{{ $t('issue_action.fissionReview') }}</p>
                <!-- 问题裂变信息按钮 -->
                <a-button
                  ref="saveButton"
                  style="marginLeft: 8px"
                  class="saveBtn fission-detail-btn fissionReview"
                  @click="findFission()"
                >
                  <!-- 问题裂变信息 -->
                  {{ $t('fissionApplication.fissonDetailButton') }}
                </a-button>
              </div>
              <div v-if="pagePermission.A2_1_3&&!isPission">
                <div
                  class="triangle_border_up"
                  style="left:388px;"
                >
            ...
            <div
              v-if="backCurrent===2&&((pagePermission.A2_1_2&&(statusCode.statusNewCode>300104||statusCode.statusNewCode<300100))||pagePermission.A2_5_3)"
            ...
            >
              ...
              <div v-if="pagePermission.A2_1_2&&!isPission">
                <a-row>
                  <!-- 根本原因 -->
                  <a-col :span="21">
              ...
              <div v-if="pagePermission.A2_1_2&&isPission&&statusCode.statusNewCode>300104">
                <a-row>
                  <a-col
                    :span="24"
                    style="text-align:center;"
                  >
                    <!-- 子问题原因分析中 -->
                    <p>
                      {{ $t('issue_reason.sub_analysised') }}
                    </p>
                  </a-col>
                </a-row>
              </div>
              <div
                v-if="(stepCurrent===backCurrent)&&(pagePermission.A2_5_3)"
                class="clearfix"
              >
                <!-- 编辑按钮 -->
                <a-button
                  ...
                  class="saveBtn fission-detail-btn fissionEdit"
                  @click="editFission()"
                >
                  <!-- 编辑 -->
                  {{ $t('issue_action.edit') }}
            ...
            <div
              v-if="(stepCurrent===3&&backFlag===false)&&((pagePermission.A3_3_2&&(backType==='003'||backType==='004'||backType==='005'))||(pagePermission.A3_1_3&&!isPission))"
            ...
            >
              ...
              <div v-if="pagePermission.A3_1_3&&!isPission">
                <div class="Dtitle">
                  <!-- 措施制定 -->
            ...
            >
              <div v-if="pagePermission.A3_1_2&&!isPission&&statusCode.statusNewCode!=400100">
                <div
                  class="triangle_border_up"
                  style="left:530px;"
                >
            ...
            <div
              v-if="(stepCurrent===4&&backFlag===false)&&((pagePermission.A4_3_2&&(backType==='003'||backType==='004'||backType==='005'))||(pagePermission.A4_1_3&&!isPission))"
            ...
            >
              ...
              <div v-if="pagePermission.A4_1_3&&!isPission">
                <!-- 短期效果 -->
                <a-row>
                  <a-col :span="21">
                    <a-form-item
            ...
            >
              <div v-if="pagePermission.A4_1_2&&!isPission">
                <div
                  class="triangle_border_up"
                  style="left:670px;"
                >
              ...
              <div v-if="pagePermission.A4_1_2&&isPission">
                <a-row>
                  ...
                  >
                    <!-- 子问题措施实施填写已结束 -->
                    <p>
                      {{ $t('issue_reason.sub_implementend') }}
                    </p>
                  </a-col>
                </a-row>
              </div>
            ...
            >
              ...
              <div v-if="pagePermission.A5_1_3">
                ...
                <a-row>
                  <a-col :span="21">
                    <a-form-item
                      :label="$t('issue_workflow.D5.problemSpread')"
                      self-update
                    >
                      <v-textarea
                        v-decorator="[
                          'horizontalExhibition'
                        ]"
                        :placeholder="$t('search.please_input')"
                        style="width:572px;"
                        :limit="1000"
            ...
            >
              ...
              <div v-if="pagePermission.A5_1_2">
                ...
                <a-row>
                  <a-col :span="21">
                    <a-form-item :label="$t('issue_workflow.D5.problemSpread')">
                      <p>{{ stepEffect.horizontalExhibition }}</p>
                    </a-form-item>
                  </a-col>
                </a-row>
                <!-- 附件 -->
            ...
            <!-- 正常关闭 -->
            <div
              v-if="stepCurrent===6&&pagePermission.A6_1_3&&backFlag===false&&!statusSpecialClose"
            ...
            >
              <div v-if="pagePermission.A6_1_3">
                ...
                <a-row>
                  <a-col :span="21">
                    <a-form-item :label="$t('issue_workflow.D6.addPreventList')">
                      <a-radio-group
                        v-decorator="[
                          'recurrencePrevention',
                          {rules: [{ required: true, message: $t('search.please_select')+$t('issue_workflow.D6.addPreventList') }]}
                        ]"
                        :options="optionRadio"
                      />
                    </a-form-item>
                ...
                <a-row>
                  <a-col :span="21">
                    <a-form-item :label="$t('issue_workflow.D6.needReview')">
                      <a-radio-group
                        v-decorator="[
                          'isSign',
                          {rules: [{ required: true, message:$t('search.please_select')+$t('issue_workflow.D6.needReview') }]}
                        ]"
                        :options="optionRadio"
                ...
                <a-row v-if="record.isSign=='1'">
                  <a-col :span="21">
                    <!-- 加签领导 -->
                    <a-form-item :label="$t('issue_workflow.D6.leader')">
                      <net-select
            ...
            >
                  ...
                  <a-row>
                    <a-col :span="21">
                      <a-form-item :label="$t('issue_workflow.attachment')">
                        <!-- 特殊关闭附件 -->
                        <div
                          v-if="specialFile && specialFile.length"
                          class="ant-upload-list ant-upload-list-text"
                          style="overflow-y: auto; max-height: 82px; margin-top: 8px; z-index: 10;"
                        >
                          <div
                            v-for="(file, index) in specialFile"
                            :key="index"
                            class="ant-upload-list-item ant-upload-list-item-done"
                            style="margin-top: 0; margin-right: 14px;"
                          >
              ...
              <!-- 重复关联关闭 -->
              <div v-if="backType==='006'">
                <!-- 标题 -->
                <div class="Dtitle examineTitle">
                  <span>{{ $t('issue_action.duplicateAssociation') }}</span>
                </div>
                <!-- 重复关联关闭理由 -->
                <a-row>
                  <a-col :span="21">
                    <a-form-item :label="$t('issue_workflow.D6.closeReason')">
                      <p>{{ examineReason }}</p>
                    </a-form-item>
                  </a-col>
                </a-row>
                ...
                <a-row>
                  <a-col :span="21">
                    <a-form-item :label="$t('issue_workflow.D6.closeTime')">
                      <p>{{ OPERETION_DATE }}</p>
                    </a-form-item>
                  </a-col>
                </a-row>
                ...
                <a-row>
                  <a-col :span="21">
                    <a-form-item :label="$t('issue_workflow.D6.closePerson')">
                      <p>{{ OP_USER_NAME }}</p>
                    </a-form-item>
                  </a-col>
                </a-row>
                <a-row>
                  <a-col :span="21">
                    <a-form-item :label="$t('issue_workflow.attachment')">
                      <!-- 重复关联附件 -->
                      <div
                        v-if="d6File && d6File.length"
                        class="ant-upload-list ant-upload-list-text"
                        style="overflow-y: auto; max-height: 82px; margin-top: 8px; z-index: 10;"
                      >
                        <div
                          v-for="(file, index) in d6File"
                          :key="index"
                          class="ant-upload-list-item ant-upload-list-item-done"
                          style="margin-top: 0; margin-right: 14px;"
                        >
              ...
              <div v-if="pagePermission.A6_1_2&&!statusSpecialClose">
                <div
                  class="triangle_border_up"
                  style="left:950px;"
                >
                ...
                <div class="Dtitle">
                  <!-- 问题关闭 -->
                  <span>{{ $t('issue_workflow.D6.processName') }}</span>
                </div>
                <!-- 关闭人 -->
                <a-row>
                  <a-col :span="21">
                ...
                <a-row>
                  <a-col :span="21">
                    <a-form-item :label="$t('issue_workflow.D6.addPreventList')">
                      <p>{{ stepClose.recurrencePrevention==='1'? $t('issue_workflow.yes'):$t('issue_workflow.no') }}</p>
                    </a-form-item>
                  </a-col>
                </a-row>
              </div>
              <div
                v-if="pagePermission.A6_2_3&&!statusSpecialClose"
                class="examine"
              >
                <!-- <div class="Dtitle">
              ...
              <div
                v-if="pagePermission.A6_2_2&&record.isSign=='1'&&!statusSpecialClose"
                class="examine"
              >
                <div class="Dtitle">
                  <!-- 加签审阅 -->
                  <span>{{ $t('issue_workflow.D6.plusReview') }}</span>
                </div>
                <a-row>
                  <a-col :span="21">
                    <!-- 审阅领导 -->
                    <a-form-item :label="$t('issue_workflow.D6.reviewLeadership')">
                ...
                <a-row>
                  <a-col :span="21">
                    <!-- 审阅时间 -->
                    <a-form-item :label="$t('issue_workflow.D6.reviewTime')">
                ...
                <a-row>
                  <a-col :span="21">
                    <!-- 备注 -->
                    <a-form-item :label="$t('issue_workflow.remark')">
      ...
      <!-- 问题裂变 -->
      <a-form
        v-if="isPission&&(statusCode.statusNewCode>=300104||statusCode.statusNewCode==300100)&visibleFission"
        class="shadown-block-normal ant-advanced-search-form"
        style="padding-top: 4px;"
      >
        <!-- 裂变问题 -->
        ...
        >
          ...
          <a-button
            ...
            class="saveBtn fission-detail-btn"
            @click="findFission()"
          >
            <!-- 问题裂变信息 -->
            {{ $t('fissionApplication.fissonDetailButton') }}
...
export default {
  ...
  components: {
    ...
    DuplicateAssociation: () => import('./DuplicateAssociation.vue'),
    ProblemFission: () => import('./ProblemFission.vue'),
    ProblemFissionView: () => import('./ProblemFissionView.vue'),
    FissionBack: () => import('./FissionBack.vue'),
    // 更换SM
    ReplaceSm: () => import('./ReplaceSM.vue'),
  },
  ...
  data () {
    const vm = this;
    return {
      standardVisiable: true, // 7钻分析查看弹框是否回显标标准要求
      ...
      visibleFission: false, // 问题裂变表格是否显示
      duplicateId: '', // 重复问题Id
      updateDataForm: null,
      d0Form: {}, // 保存D0表单
      ...
      // 问题裂变
      isPission: false, // 该问题是否裂变标识
      isFission: '0', // 问题裂变申请类型
      masterIssueId: '', // 主问题id
      // 回退
      backType: '', // 回退标识
      OPERETION_DATE: '', // 操作时间
      ...
      diamondOwner1: '', // 初始化1-3钻责任人
      diamondOwner4: '', // 初始化4钻责任人
      diamondOwner5: '', // 初始化5钻责任人
      diamondOwner6: '', // 初始化6钻责任人
      diamondOwner7: '', // 初始化7钻责任人
      workflowRolesList: [],
      workflowRoles: {
        createUser: '',
        advanceUser: '',
      ...
      record: {
        ...
        endSeven: '0',
        // 7钻责任部门id
        responsibleDepartmentId1: '',
        responsibleDepartmentId4: '',
        responsibleDepartmentId5: '',
        responsibleDepartmentId6: '',
        responsibleDepartmentId7: '',
        diamondOwner1: '',
        diamondOwner4: '',
        diamondOwner5: '',
        diamondOwner6: '',
  ...
  methods: {
    ...
    mapPropsToFieldsForm () {
      return createFormFields(this, [
        ...
        'owerDeptLv1', 'champion', 'cochairDepartment', 'cochair', 'type', 'responsibleDepartmentId1', 'responsibleDepartmentId4', 'responsibleDepartmentId5', 'responsibleDepartmentId6', 'responsibleDepartmentId7', 'diamondOwner1', 'diamondOwner4', 'diamondOwner5',
        ...
        'icaExecDescription', 'icaExecTime', 'pcaExecDescription', 'lessonsLearn', 'horizontalExhibition',
    ...
    // 查看裂变信息
    findFission () {
      this.$refs.fissionView.init(this.duplicateId);
    },
    // 点击重复关联问题问题编号跳转链接
    gotoDetail (row) {
    ...
    // 点击问题裂变按钮
    fissionCreate () {
      this.$refs.fissionApplication.init(this.statusCode.statusNewCode === '300100' || this.statusCode.statusNewCode === '300300' || this.statusCode.statusNewCode === '300500' || this.statusCode.statusNewCode === '300700' || this.statusCode.statusNewCode === '300900' ? 'create' : 'edit', this.duplicateId, this.detailList, this.statusCode.statusNewCode, true);
    },
    // 点击裂变信息按钮
    editFission () {
      if (this.pagePermission.A2_5_3) {
        this.$refs.fissionApplication.init('edit', this.duplicateId, this.detailList, this.statusCode.statusNewCode, false);
      } else if (this.pagePermission.A2_5_2) {
        this.$refs.fissionApplication.init('edit', this.duplicateId, this.detailList, this.statusCode.statusNewCode, false);
      }
    },
    // 点击更换SM按钮
    replaceSMFunction () {
      this.$refs.replaceSM.init(this.id, this.workflowRoles, this.detailList, this.statusCode.statusNewCode);
    },
    resMapPropsToFields () {
      return createFormFields(this, [
        'owerDeptLv1', 'champion', 'rediStributionMessage',
      ], 'redistributionForm');
    },
    ...
    rollback () {
      if (!this.isPission) {
        ...
        this.$refs.fissionBack.init(this.id, this.taskId, this.statusCode.statusNewCode, this.masterIssueId);
      }
    },
    // 特殊关闭
    specialClose () {
    ...
    DeptChange (param) {
      ...
      if (param === 1) {
        this.record.diamondOwner1 = void 0;
      } else if (param === 4) {
        this.record.diamondOwner4 = void 0;
      } else if (param === 5) {
        this.record.diamondOwner5 = void 0;
      } else if (param === 6) {
        this.record.diamondOwner6 = void 0;
      } else if (param === 7) {
        this.record.diamondOwner7 = void 0;
      }
    ...
    init () {
      ...
      this.rediStribution = this.$form.createForm(this, {
        mapPropsToFields: () => createFormFields(this, ['owerDeptLv1', 'champion', 'rediStributionMessage'],
          'redistributionForm'),
    ...
    showAnalysis (param) {
      ...
      if (index > 3) {
        this.standardVisiable = false;
      } else {
        this.standardVisiable = true;
      }
    ...
    request () {
      ...
      const editDetail = this.eidtQuestion(this.id).then((res) => {
        ...
        // 是否裂变问题
        this.isPission = res.isPission === '1';
        // 主问题id
        this.masterIssueId = res.masterIssueId;
        this.statusSpecialClose = res.statusSpecialClose !== null;
      ...
      Promise.all([editDetail, pagePermissionArea]).then((res) => {
        ...
        this.getActIdMembership(transData).then((ress) => {
          if (ress && Object.keys(ress).length > 0) {
            ...
            // 7钻初始化责任人赋值
            this.diamondOwner1 = ress.diamondOwner1;
            this.diamondOwner4 = ress.diamondOwner4;
            this.diamondOwner5 = ress.diamondOwner5;
            this.diamondOwner6 = ress.diamondOwner6;
            this.diamondOwner7 = ress.diamondOwner7;
    ...
    getQuestionStepAll (id) {
      ...
      this.stepCurrent === 1 && this.issueDefinition(id).then((res) => {
        ...
        if (res) {
          ...
          if (res.type === '1') {
            ...
            this.record.responsibleDepartmentId1 = res.sevenDiamondsVos[0].responsibleDepartmentId;
            this.record.responsibleDepartmentId4 = res.sevenDiamondsVos[3].responsibleDepartmentId;
            this.record.responsibleDepartmentId5 = res.sevenDiamondsVos[4].responsibleDepartmentId;
            this.record.responsibleDepartmentId6 = res.sevenDiamondsVos[5].responsibleDepartmentId;
            this.record.responsibleDepartmentId7 = res.sevenDiamondsVos[6].responsibleDepartmentId;
            ...
            // 责任部门
            this.issueDefinitionData.responsibleDepartmentName1 = res.sevenDiamondsVos[0].responsibleDepartmentName;
            this.issueDefinitionData.responsibleDepartmentName4 = res.sevenDiamondsVos[3].responsibleDepartmentName;
            this.issueDefinitionData.responsibleDepartmentName5 = res.sevenDiamondsVos[4].responsibleDepartmentName;
            this.issueDefinitionData.responsibleDepartmentName6 = res.sevenDiamondsVos[5].responsibleDepartmentName;
            this.issueDefinitionData.responsibleDepartmentName7 = res.sevenDiamondsVos[6].responsibleDepartmentName;
          }
        ...
        if (pagePermission.A1_1_2 && (status >= 300100 || status === 200105)) {
          ...
          // eslint-disable-next-line no-plusplus
            ...
            this.redistributionForm.owerDeptLv1 = this.issueDefinitionData.sevenDiamondsVos[2].responsibleDepartmentId;
          }
      ...
      this.stepCurrent === 5 && this.effectDetail(this.id).then((res) => {
        ...
        this.record.horizontalExhibition = res.horizontalExhibition;
    ...
    showModal () {
      ...
      const status = Number(this.detailList.status);
      if (status >= 200200 && status < 200500) {
        this.redistributionForm.owerDeptLv1 = this.issueDefinitionData.sevenDiamondsVos[0].responsibleDepartmentId;
      } else if (status >= 200500 && status < 200700) {
        this.redistributionForm.owerDeptLv1 = this.issueDefinitionData.sevenDiamondsVos[3].responsibleDepartmentId;
      } else if (status >= 200800 && status < 201000) {
        this.redistributionForm.owerDeptLv1 = this.issueDefinitionData.sevenDiamondsVos[4].responsibleDepartmentId;
      } else if (status >= 201100 && status < 201300) {
        this.redistributionForm.owerDeptLv1 = this.issueDefinitionData.sevenDiamondsVos[5].responsibleDepartmentId;
      } else if (status >= 201400 && status < 201600) {
        this.redistributionForm.owerDeptLv1 = this.issueDefinitionData.sevenDiamondsVos[6].responsibleDepartmentId;
      }
      this.rediStribution.updateFields(this.resMapPropsToFields());
    },
    handleOk () {
    ...
    handleCancel () {
      ...
      // 将7钻责任人还原
      this.workflowRoles.diamondOwner1 = this.diamondOwner1;
      this.workflowRoles.diamondOwner4 = this.diamondOwner4;
      this.workflowRoles.diamondOwner5 = this.diamondOwner5;
      this.workflowRoles.diamondOwner6 = this.diamondOwner6;
      this.workflowRoles.diamondOwner7 = this.diamondOwner7;
      enableScroll();
    },
    // 是否需要7钻分析
    determineChange (e) {
    ...
    handleUser () {
      ...
      this.rediStribution.validateFields((err) => {
        if (!err) {
          ...
          let param = {};
          // 1~3 钻
          if (this.statusCode.statusNewCode === '200200' || this.statusCode.statusNewCode === '200400') {
            param = {
              taskId: this.detailList.taskId,
              variable: {
                diamondOwner1: data.champion,
              },
            };
            this.workflowRoles.diamondOwner1 = data.champion;
          } else if (this.statusCode.statusNewCode === '200500' || this.statusCode.statusNewCode === '200700') {
            // 4钻
            param = {
              taskId: this.detailList.taskId,
              variable: {
                diamondOwner4: data.champion,
              },
            };
            this.workflowRoles.diamondOwner4 = data.champion;
          } else if (this.statusCode.statusNewCode === '200800' || this.statusCode.statusNewCode === '201000') {
            // 5钻
            param = {
              taskId: this.detailList.taskId,
              variable: {
                diamondOwner5: data.champion,
              },
            };
            this.workflowRoles.diamondOwner5 = data.champion;
          } else if (this.statusCode.statusNewCode === '201100' || this.statusCode.statusNewCode === '201300') {
            // 6钻
            param = {
              taskId: this.detailList.taskId,
              variable: {
                diamondOwner6: data.champion,
              },
            };
            this.workflowRoles.diamondOwner6 = data.champion;
          } else if (this.statusCode.statusNewCode === '201400' || this.statusCode.statusNewCode === '201600') {
            // 7钻
            param = {
              taskId: this.detailList.taskId,
              variable: {
                diamondOwner7: data.champion,
              },
            };
            this.workflowRoles.diamondOwner7 = data.champion;
          } else {
            param = {
              taskId: this.detailList.taskId,
              variable: {
    ...
    closeOk () {
      ...
      this.closeForm.validateFields((err) => {
        if (!err) {
          ...
          this.closeSave(paramClose).then();
          this.speCloseWorkFlow(transData).then((res) => {
            this.fissionSubIssueClose();
    ...
    submitOk () {
      ...
      // 裂变子问题提交
      if (this.statusCode.statusNewCode === '300102' && this.record.isPass === '1') {
        this.handleFissionSubmitFunction();
      }
    ...
    handleSubmitFunction () {
      ...
      this.formDcontent.validateFields((err) => {
        if (!err) {
          ...
          // 裂变操作码
          if (this.isFission === '1' || this.isFission === '2') {
            this.operationCode = this.isFission === '1' ? '007' : '008';
          }
          ...
          const transData = {
            ...
            variables: {
              businessKey: this.id, // 问题id
              comment: commentVal || '',
              isFission: this.isFission, // 裂变操作类型
              assigner: data.diamondOwner1,
    ...
    handleSave (status) {
          ...
          if (!_this.analysisData.length || this.statusCode.statusNewCode === '200105') {
            if (!this.NeedFlage) {
              _this.analysisData.push({
                ...
                responsibleDepartmentId: this.record.responsibleDepartmentId1,
                type: 'DIAMONDS01',
            ...
            if (!this.NeedFlage) {
              _this.analysisData.push({
                ...
                responsibleDepartmentId: this.record.responsibleDepartmentId4,
                type: 'DIAMONDS04',
            ...
            if (!this.NeedFlage) {
              _this.analysisData.push({
                ...
                responsibleDepartmentId: this.record.responsibleDepartmentId5,
                type: 'DIAMONDS05',
            ...
            if (!this.NeedFlage) {
              _this.analysisData.push({
                ...
                responsibleDepartmentId: this.record.responsibleDepartmentId6,
                type: 'DIAMONDS06',
            ...
            if (!this.NeedFlage) {
              _this.analysisData.push({
                ...
                responsibleDepartmentId: this.record.responsibleDepartmentId7,
                type: 'DIAMONDS07',
      ...
      if (this.stepCurrent === 3 && this.pagePermission.A3_1_3) {
        ...
        if (this.pagePermission.A5_4_3) {
          ...
          data.horizontalExhibition = this.stepEffect.horizontalExhibition;
        }
    ...
    // 查看来源模块详情
    moduleSourceDetail () {
      if (this.detailList.moduleSource === 'ASQ') {
        window.open(`/aqs/issue/definition-list/report/${this.detailList.moduleSourceId}`);
      }
    },
    // 是否满足立项条件切换
    satisfyChange (e) {
    ...
    cancelFission () {
      this.isFission = '2';
      this.issueIsPissionFunction();
      this.handleSubmitFunction();
    },
    // 主问题问题裂变修改
    issueIsPissionFunction () {
      const param = {
        id: this.id,
        isPission: '0',
      };
    ...
    // 处理问题裂变提交
    handleFissionSubmitFunction () {
      const paramChild = {
        issueId: this.id,
      };
      this.childSave(paramChild).then((res2) => {
        res2.forEach((subIssue) => {
          const transData = {
            businessKey: subIssue.id, // 子问题id
            businessTitle: subIssue.title, // 问题title
            ...
            variables: {
              businessKey: subIssue.id, // 子问题id
              comment: '',
              champion: subIssue.assigner,
              monitor: subIssue.responsibleUserId,
            },
          };
          this.workFlowSubmitNew(transData).then(() => {
          });
    ...
    // 子问题关闭
    fissionSubIssueClose () {
      if (this.isPission) {
        this.getStatusCode(this.id).then((masterIssueStatus) => {
          if (masterIssueStatus) {
            if (masterIssueStatus.statusNewCode === '700300') {
              const param = {
                masterIssueId: this.id,
                exclude: this.id,
              };
              this.associatedTable(param).then((subIssueList) => {
                subIssueList.forEach((subIssue) => {
                  const transData = {
                    businessKey: subIssue.id, // 子问题id
                    businessTitle: subIssue.title, // 问题title
                    processDefinitionKey: 'IRS3', // IRS2  固定值
