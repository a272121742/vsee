...
export default {
  ...
  computed: {
    order () {
      return defaultTo('', this.advancePageConfig.doneOrderData.order).slice(0, -3);
    },
    orderField () {
      return this.advancePageConfig.doneOrderData.field;
    },
    page () {
      return this.advancePageConfig.donePageData.current;
    },
    limit () {
      return this.advancePageConfig.donePageData.pageSize;
    },
    columns () {
      const orderData = this.advancePageConfig.doneOrderData;
      return issueDoneColumns.map(col => {
  ...
  methods: {
    ...
    handleTableChange ({ current = 1, pageSize = 10 }, filters, { order = '', field = '' }) {
      // 缓存数据到store
      this.changeAdvancePageConfig({ doneOrderData: { order, field } });
      this.changeAdvancePageConfig({ donePageData: { current, pageSize } });
      this.request();
    },
    /**
     * 搜索（根据表单传过来的条件）
