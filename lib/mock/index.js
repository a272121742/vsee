...
const mock = (json, _count) => {
  let count = 1;
  let returnArray = false;
  if (typeof _count === 'number' && ~_count && Number.isFinite(_count)) {
    count = _count;
    returnArray = true;
  }
  const key = 'key';
  ...
  const result = Mock.mock(convertJSON)[key];
  return returnArray ? (count === 1 ? [result] : result) : result;
};
const httpMethods = ['get', 'post', 'patch', 'put', 'delete', 'head', 'options'];

const useParams = (type) => ['get', 'head', 'delete', 'options'].includes(type);
...
httpMethods.forEach((type) => {
  ...
  mock[type] = (urlReqExpString, callback, {
    ...
    const rurl = pathToRegexp(urlReqExpString, [], {
      // 不忽略大小写
      sensitive: true,
      strict: true,
    });
    ...
    AdapterMock[mockMethod](rurl).reply(async (config) => {
      ...
      args.push(['get', 'head', 'delete', 'options'].includes(type) ? params : datas);
