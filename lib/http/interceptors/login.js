...
const getCookieConfig = pick(['expires', 'path', 'domain', 'secure']);
  ...
  axios.interceptors.response.use(tapclone((res) => {
    ...
    if (res.config && ~res.config.url.indexOf(AUTH_LOGIN_API.URL)) {
      ...
      if (token) {
        ...
        if (!TOKEN_CUSTSET) {
          const cookieConfig = getCookieConfig(data);
          cookieConfig.expires = Math.max(Math.min(parseInt(cookieConfig.expires, 10) || 7, 365), -1);
