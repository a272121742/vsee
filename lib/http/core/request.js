...
interceptorsReq
  ...
  .forEach((mod) => {
    ...
    console.log('使用代理吗？', mod, useProxy);
...
export default {
  ...
  post (url, data, conf = {}) {
    const http = store.state.isMock ? proxy : axios;
    return http.post(url, data, { ...conf }).then(resolved).catch(rejected);
  },
  put (url, data, conf = {}) {
    const http = store.state.isMock ? proxy : axios;
    return http.put(url, data, { ...conf }).then(resolved).catch(rejected);
  },
  patch (url, data, conf = {}) {
    const http = store.state.isMock ? proxy : axios;
    return http.patch(url, data, { ...conf }).then(resolved).catch(rejected);
  },
  options (url, params, conf = {}) {
    const http = store.state.isMock ? proxy : axios;
    return http.options(url, { ...conf, params }).then(resolved).catch(rejected);
  },
  request (conf) {
    // const http = store.state.isMock ? proxy : axios;
