...
export default {
  ...
  post (url, data, conf = {}) {
    const http = store.state.isMock ? proxy : axios;
    return http.post(url, data, { ...conf, data }).then(resolved).catch(rejected);
  },
  put (url, data, conf = {}) {
    const http = store.state.isMock ? proxy : axios;
    return http.put(url, data, { ...conf, data }).then(resolved).catch(rejected);
  },
  patch (url, data, conf = {}) {
    const http = store.state.isMock ? proxy : axios;
    return http.patch(url, data, { ...conf, data }).then(resolved).catch(rejected);
  },
