...
import axios, { baseURL, paramsSerializer } from './core';
...
export default {
  ...
  getHeaders () {
    return omit(['get', 'post', 'patch', 'put', 'delete', 'head', 'common'], axios.defaults.headers);
  },
  // 服务下载
  download (url, params = {}, config = {}) {
    const custHeader = config.headers || {};
    const updateHeaders = this.getHeaders();
    return new Downloader({
      url: `${baseURL}/${url}?${paramsSerializer(params)}`.replace(/\/+/g, '/').replace(/:\//g, '/'),
      headers: map2nameValue({ ...updateHeaders, ...custHeader }),
