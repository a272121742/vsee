...
function setStyle ($tr, finallIndex, callback1, callback2) { // callback1:默认样式设置,callback2:拖动的时候样式
  for (let tri = 0; tri < $tr.length; tri += 1) { // 遍历tr,将th和td放到最终的位置上
    const itemtr = $tr[tri];
    if (itemtr.getElementsByTagName('th').length) {
      const $ths = itemtr.getElementsByTagName('th');
      for (let thi = 0; thi < $ths.length; thi += 1) {
    ...
    if (itemtr.getElementsByTagName('td').length) {
      ...
      for (let tdi = 0; tdi < $tds.length; tdi += 1) {
...
Vue.directive('drag-table', {
  bind: (el) => {
    ...
    op.onmousedown = (e) => {
      ...
      for (let thi = 0; thi < $th.length; thi += 1) {
        const itemth = $th[thi];
        itemth.index = thi;
        let offsetWidth = 0;
        if (thi === 0) {
      ...
      document.onmousemove = (e) => {
        ...
        if (left <= 0) { // // 小于0的时候就等于0
          ...
          for (let i = 0; i < thWidthArr.length; i += 1) {
      ...
      document.onmouseup = (e) => {
        ...
        // 遍历tr,将th和td放到最终的位置上
        for (let tri = 0; tri < $tr.length; tri += 1) {
          const itemtr = $tr[tri];
          if (itemtr.getElementsByTagName('th').length) {
            const $ths = itemtr.getElementsByTagName('th');
