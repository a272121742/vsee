...
if (process.env.NODE_ENV !== 'production') {
  ...
  Axios.defaults.baseURL = 'http://106.75.63.69:8091/mojo-gateway';
}
...
const resolve = res => {
  if (res.data instanceof ArrayBuffer) {
    return res.data;
  } else if (Object.prototype.hasOwnProperty.call(res.data, 'data')) {
    return res.data.data;
  } else {
    return res.data;
  }
  // return isDevProxyTesting ? res.data : res.data.data;
};
const reject = err => Promise.reject(err);

console.info('当前模式', isDevProxyTesting ? '联调开发模式：' + process.env.proxyServerPix : '离线开发模式：/');
const axios = isDevProxyTesting ? proxyAxios : Axios;
axios.defaults.headers.token = Cookies.get('login_token');
...
function afterLogin (axios) {
  ...
  axios.interceptors.response.use(tap(res => {
    ...
    if (~res.config.url.indexOf('login')) {
      // TODO: 是从res.data中获取，还是从res.headers中获取 ?
      const token = (res.data && res.data.data && res.data.data.token);
      const expire = (res.data && res.data.data && res.data.data.expire) || 7;
      if (token) {
        Cookies.remove('login_token');
        Cookies.set('login_token', token, { expires: expire });
        axios.defaults.headers.token = token;
        // TODO: 这里是适配`resolve`获取数据的修改
        // res.data.data = { token, result: res.data.data };
      } else {
        console.error('非法操作, 系统将在5秒后自动跳转到登陆页面');
      }
    }
  }));
}
