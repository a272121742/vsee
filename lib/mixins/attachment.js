...
export default {
  ...
  methods: {
    ...
    getDownloadURL (url, name) {
      ...
      const encodePath = url; // window.encodeURIComponent(url);
      const encodeFileName = name; // window.encodeURIComponent(name);
    ...
    download (file) {
      if (file.status === 'done') {
        ...
        // download(this.getDownloadURL(file.url || file.response.data.path, file.name));
      }
      return false;
    },
    beforeUpload (file) {
