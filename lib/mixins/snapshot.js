...
export default {
  install (Vue) {
    ...
    Vue.prototype.$snapshot = (command, dataName, version = 'default') => {
      // 获取组件实例
      const vm = this;
      // 注册组件到快照中
      if (!Object.hasOwnProperty.call(SnapshotMemory, vm._uid)) {
        SnapshotMemory[vm._uid] = {};
      }
      // 获取组件快照
      const snapshot = SnapshotMemory[vm._uid];
      // 获取组件数据，computed不获取
      ...
      case 'set':
        ...
        if (~dataKey.indexOf(dataName)) {
          // 如果该属性快照不存在，初始化一个
          if (!Object.hasOwnProperty.call(snapshot, dataName)) {
            snapshot[dataName] = {};
          }
      ...
      case 'restore':
        if (~dataKey.indexOf(dataName)) {
          if (!Object.hasOwnProperty.call(snapshot, dataName) || !Object.hasOwnProperty.call(snapshot[dataName], version)) {
