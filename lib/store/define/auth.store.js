...
const allowLogin = SKIP_LOGIN ? false : require.context('~', true, /^\.\/login\/view\/Login\.vue/).keys().length === 1;
  ...
  actions: {
    login ({ commit }, loginInfo) {
      return new Promise((resolve, reject) => {
        if (AUTH_LOGIN_API && AUTH_LOGIN_API.URL) {
    ...
    logout ({ state, commit }) {
      return new Promise((resolve, reject) => {
        ...
        } else if (AUTH_LOGOUT_API && AUTH_LOGOUT_API.URL) {
