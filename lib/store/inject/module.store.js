...
export default function (moduleExport = {}) {
  ...
  if (hasOwnProperty(moduleStore, 'state')) {
    ...
    moduleStore.getters.getState = () => () => moduleExport.state;
    ...
    moduleStore.mutations.reset = reset;
    moduleStore.mutations.fill = reset;
    moduleStore.mutations.fillState = reset;
    // 设置（仅设置当前属性，不改变其他属性）
    ...
    moduleStore.mutations.setState = set;
    // 还原（还原全部属性）
    const init = (state) => {
      forEachObjIndexed((value, key) => {
    ...
    moduleStore.mutations.initState = init;
    // 自定义更新
