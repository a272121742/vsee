...
const localRouters = routeRequest(autoRequires, excludeModules, contentComp);
const project = process.env.project;
const base = process.env.NODE_ENV === 'development' ? '/' : `/${project}`;
...
if (homeComp) {
  // localRouters.systemRouters.push(homeRouter[0]);
}
homeRouter[0].children.push(...localRouters.appRouters);
...
function routeRequest (autoRequires, excludeModules, contentComp) {
  const projectPath = '.' + autoRequires.router.id.split(' ')[0];
