...
const THEME = (() => {
  ...
  return {
    DARK, LAYOUT_TYPE, MENU_TYPE, CONTENT_HEAD, TAB_TYPE, MENU_COLLAPSIBLE, MENU_COLLAPSED, MENU_CURRENT, KEEP_ALIVE, BACK_TOP: !!BACK_TOP,
  };
})();
...
const ROUTER = (() => {
  ...
  if (configured(PORTAL_COMP) && !(PORTAL_COMP instanceof Function)) {
    throwConfigError('必须是一个返回组件的函数', 'ROUTER', 'PORTAL_COMP');
  }
  return {
    HOME_COMP, PORTAL_COMP, MODE, CUSTOM, INJECT,
  };
})();
...
const MESSAGE = (() => {
  ...
  if (typeof TOP !== 'number') {
    throwConfigError('必须是数字', 'MESSAGE', 'TOP');
  }
  if (typeof DURATION !== 'number') {
    throwConfigError('必须是数字', 'MESSAGE', 'DURATION');
  }
  return { TOP, DURATION, GLOBAL: !!GLOBAL };
})();
