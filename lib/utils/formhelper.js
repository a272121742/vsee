  ...
  beforeCreate () {
    this.$v = {
      ...
      maxLength (max, message) {
        ...
        return { type: 'string', transform: String, max, message: format(message, { max }) };
      },
      minLength (min, message) {
        if (notAllowNumber(min)) {
          throw new Error('【配置错误】：min必须为数字');
        }
        return { type: 'string', transform: String, min, message: format(message, { min }) };
      },
      rangeLen (range, message) {
        ...
        const [min, max] = range;
        return { type: 'string', transform: String, min, max, message: format(message, { min, max }) };
      },
      len (len, message) {
        if (notAllowNumber(len)) {
          throw new Error('【配置错误】：len必须为数字');
        }
        return { type: 'string', transform: String, len, message: format(message, { len }) };
      },
      max (max, message) {
        if (notAllowNumber(max)) {
          throw new Error('【配置错误】：max必须为数字');
        }
        return { type: 'number', transform: Number, max, message: format(message, { max }) };
      },
      min (min, message) {
        if (notAllowNumber(min)) {
          throw new Error('【配置错误】：min必须为数字');
        }
        return { type: 'number', min, transform: Number, message: format(message, { min }) };
      },
      range (range, message) {
        if (!isArray(range) || range.length !== 2) {
          throw new Error('【配置错误】：range必须为[min, max]类型数组');
        ...
        const [min, max] = range;
        return { type: 'number', transform: Number, min, max, message: format(message, { min, max }) };
      },
      equal (value = '', message) {
      ...
      // 前缀判断
      prefix (txt = '', message) {
        return { type: 'string', transform: String, pattern: new RegExp(`^${txt}`, 'g'), message: format(message, { txt }) };
      },
      // 后缀判断
      suffix (txt = '', message) {
        return { type: 'string', transform: String, pattern: new RegExp(`${txt}$`, 'g'), message: format(message, { txt }) };
      ...
      remote (url, query = {}) {
        ...
        return { asyncValidator (rule, value, callback) {
