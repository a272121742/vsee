...
import {
  tryCatch, always, forEachObjIndexed, map, path, forEach, pipe, indexBy, prop, omit, ifElse, identity, pick, merge, objOf, curry, converge, compose
} from 'ramda';
 ...
 */
export const treeFilter = curry((fn, tree = []) => tree.filter(fn,
  tree.map(item => ({ ...item, children: treeFilter(fn, item.children) }))));
