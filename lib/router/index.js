...
import { treeEach, treeFilter, tree2list } from '@util/tree-helper.js';
...
const req = require.context('~', true, /^\.(\/[A-Za-z][A-Za-z0-9_-]*)+\/router\/[~A-z0-9- ./]+\.vue$/, 'lazy');
...
store.watch((state) => state.menus, (unfilterMenus) => {
  ...
  treeEach((menu, index, path, parentNode) => {
...
if (!CUSTOM) {
  if (store.state.allowLogin) {
    router.beforeEach(async (to, from, next) => {
      NProgress.start();
      if (to.params.pathMatch && to.params.pathMatch === to.path && !to.name) {
        const id = setTimeout(() => {
          clearTimeout(id);
      ...
      if (!toLogin && !isLogin) {
        ...
        if (!Object.keys(INJECT).includes(to.path)) {
          next({
            name: 'Login',
            query: {
              redirect: to.fullPath.replace('/logout', ''),
            },
          });
        }
      }
