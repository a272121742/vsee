...
const router = new VueRouter({
  mode: config.ROUTER_MODE || 'history',
  base,
  ...
  treeEach((menu, index, parent) => {
    ...
    if (menu.url) {
      ...
      if (menu.hide) {
        ...
        if (!parent.children.find((item) => !item.path) && sameNameRouter) {
...
if (!config.CUST_ROUTER) {
  ...
  function checkRouter (to, from, next) {
    function findNext (menus = []) {
      const home = menus.reduce(function rd (p, item, index) {
        if (index === 0) {
          let path = '';
          if (item.url) {
            path += `/${item.url}`;
