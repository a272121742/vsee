...
const defaultRoutes = [...INJECT, ...systemRouters, {
  path: '*?',
...
function tree2list (tree = [], children = 'children', parent = {}) {
  return tree.reduce((list, item) => {
    const current = { ...item, fullPath: `${parent.fullPath || ''}${item.url ? `/${item.url}` : ''}` };
    if (item.children && !item.children.length) {
      current.del = true;
    }
    if (/\/:[A-Za-z][A-Za-z0-9]*\?/.test(current.url)) {
      current.hide = true;
    }
    if (/^http/.test(current.url)) {
      current.link = true;
    }
    delete current.children;
    list.push({ ...current });
    if (item[children] && item[children].length) {
      list.push(...tree2list(item.children, children, current));
    }
    return list;
  }, []);
}
...
store.watch((state) => state.menus, (unfilterMenus) => {
  ...
  router.addRoutes([{
    ...
    children: [...allowRouter,
      {
        path: '',
        ...(allowRouter[0] ? { redirect: allowRouter[0].path } : {}),
      },
      {
        path: '*',
