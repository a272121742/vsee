...
const mode = LAYOUT_TYPE === 'anchor' ? 'hash' : ROUTER_MODE;
...
if (config.LOCATION_MENU === true) {
  router.addRoutes([{
    path: '/',
    component: layoutComp,
    children: componentMap.map((item) => ({
      path: item.url,
    ...
    function findNext (menus = []) {
      ...
      const home = (mode === 'hash' || (homeComp && homeComp instanceof Function)) ? true : menus.reduce(function rd (p, item, index) {
        if (index === 0) {
          ...
          if (config.ROUTER_DEEP && item.children && item.children.length) {
    ...
    if (store.state.isLogin) {
      ...
      if (store.state.isMenusLoaded) {
        ...
        store.dispatch('fetchMenus').then((res) => {
          ...
          const locationMenu = config.LOCATION_MENU instanceof Array ? config.LOCATION_MENU : [];
          const issueMenus = treeFilter(menuFilter, locationMenu);
          store.commit('setServerMenus', issueMenus);
          Vue.nextTick(() => {
            next(findNext(locationMenu));
          });
        });
      }
