...
export default {
  name: 'BtmHugeTree',
  components: {
    'dt-checkbox': () => import('./Checkbox.vue'),
  },
  props: {
    // 含有过滤输入框
  ...
  methods: {
    ...
    setFilterList (op) {
      ...
      if (this.isOnlyInCheckedSearch && this.big.allCheckedList.length > 0) {
        ...
        this.big.filterList = this.big.allCheckedList.filter((i) => isIncludesKeyword(i, this.keyword, this.big.allCheckedList));
        return;
      }
      if (this.keyword.trim() === '') {
      ...
      this.big.filterList = this.big.list.filter((i) => isIncludesKeyword(i, this.keyword, this.big.list));
    },
    // 回到顶部
    backToTop () {
      this.$nextTick(() => {
