...
| remote({ url, query, success, trigger}) |          | 判断输入项目是否满足远程验证，若不则返回验证消息 |

远程验证`remote`的参数如下：

- url - 验证地址（发起get请求）
- query - 验证参数对象
- success - 成功后的回调函数，函数中能拿到服务端传回的数据（`response.body.data`）
- trigger - 触发方式，默认为`blur`，可选值`change`，不建议修改

远程验证时，向服务端发起验证的请求和整个平台的请求是一致的，`response.body`的返回结果一定`{data, code, msg}`，并且都会对`code`做拦截处理。如果成功返回了正确的结果，都会在`succuess`函数中捕获到，反之错误信息会在内部通过`asyncValidator.callback`的方式返回给视图，作为开发者不需要拿到错误信息。
