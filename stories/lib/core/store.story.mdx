...
有关`Vuex`的相关内容，在[官方文档](https://vuex.vuejs.org/zh)中都有详细的描述，这里不做再次的描述了。`vsee`框架中内置了`Vuex`，并对`Vuex`的加载进行了工程化改造。至于你想不想用它，完全取决于你项目的实际用途和开发模式。下面我们将进行详细叙述：
...
可以在系统加载初通过服务端接口获取系统信息，获取之后通过`mutations`的`commit`方法进行设置，即可让系统信息全局共享，相应的其他信息内容不再赘述。

```json
{
  state: {
    // 系统信息，可以存储和系统相关联的全部信息
    systemInfo: Object,
  },
  getters: {
    /**
     * 获取系统信息
     * @return Object - 返回系统信息
     */
    getSystemInfo () {} 
  },
  mutataions: {
    /**
     * 设置系统信息
     * @params Object systemInfo - 系统信息对象
     */ 
...
```json
{
  ...
  getters: {
    /**
     * 获取权限列表
     * @return Array<String> - 返回权限列表
     */
    getPermissions () {},
    /**
     * 判断是否包含目标权限
     * @params String permission - 目标权限
     * @return Boolean - 权限列表中是否包含该目标权限
     */ 
  ...
  mutataions: {
    /**
     * 设置权限列表
     * @params Array<String> permissions - 权限列表
     */ 
...
{
  state: {
    // 工作流对象，可以存储服务端全部工作流信息
    workflow: Object,
  },
  getters: {
    /**
     * 获取工作流对象
     * @return Object - 返回工作流对象
     */
    getWorkflow () {},
  },
  mutataions: {
...
```json
{
  ...
  mutations: {
     ...
     * 填充状态树
     * 1. 填充时，会还原其他属性，再设置属性
     * 2. 填充的数据如果未出现在state的定义中，则不填充该属性
     * 3. 同名方法还有`reset`和`fill`
     ...
     * 3. 同名方法还有`update`和`set`
     * @params Object payload - 设置对象
     */ 
    setState (payload) {},
    /**
     * 还原状态树
     * 1. 还原时，直接将状态树还原为最初始状态
     * 2. 同名方法还有`init`
     */ 
    initState () {},
  }
